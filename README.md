...
脚本放到 GPT-SoVITS 项目根目录 (docker: `/workspace/GPT-SoVITS/`)。
...
visqol 需要评估的音频和原始音频是相同采样率，需要先将 `input/*.wav` 的所有 48k 原始音频用 ffmpeg 转为 32k，生成 `input/<filename>_32k.wav` 版本（忽略已经是 `_32k.wav` 后缀的文件；静默覆盖已存在文件）。
